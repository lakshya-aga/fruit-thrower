...
def configure_authenticated_remote(
    repo_root: Path, repo_slug: str, token_env: str = "AGENT_GITHUB_TOKEN"
) -> bool:
    """Set authenticated HTTPS origin for push if token env is present."""
    token = (
        os.environ.get(token_env, "").strip()
        or os.environ.get("AGENT_GITHUB_TOKEN", "").strip()
    )
    if not token:
        return False
...
def apply_changes(
    spec: dict, request_id: str, repo_root: Path, module_rel: str
) -> None:
...
def maybe_create_pr(
    request_id: str, tool_name: str, repo_slug: str, base: str
) -> str | None:
...
def main() -> None:
    ...
    if module_path.startswith("fin-kit/"):
        ...
        repo_slug = os.environ.get(
            "AGENT_GITHUB_REPO_FIN_KIT", "lakshya-aga/fin-kit"
        ).strip()
        ...
        repo_slug = os.environ.get(
            "AGENT_GITHUB_REPO", "lakshya-aga/fruit-thrower"
        ).strip()
    ...
    run(
        [
            ...
            "commit",
            "-m",
            f"agent: implement requested function {spec['tool_name']} ({request_id})",
        ],
        cwd=repo_root,
    )
    ...
    if args.push:
        used_auth_remote = configure_authenticated_remote(
            repo_root=repo_root, repo_slug=repo_slug, token_env=token_env
        )
    ...
    pr_url = (
        maybe_create_pr(
            request_id, spec["tool_name"], repo_slug=repo_slug, base=pr_base
        )
        if (args.push and auto_pr)
        else None
    )
    report["branch"] = "agent"

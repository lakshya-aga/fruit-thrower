...
def module_sidebar(modules: list[tuple[str, dict]], current: str, out_dir: Path) -> str:
    ...
    for name, counts in sorted(modules):
        ...
        items.append(
            f'<li><a href="{href}"{cls}>{name}<span class="badge">{n}</span></a></li>'
        )
...
def render_unit(u: dict) -> str:
    ...
    sig_html = (
        f'<div class="signature-block">{_esc(sig)}</div>'
        if sig and kind != "module"
        else ""
    )
    doc_html = (
        f'<div class="docstring">{_esc(doc)}</div>'
        if doc
        else '<div class="no-doc">No docstring.</div>'
    )
...
{"".join(cards)}"""
    ...
    for mod, units in sorted(by_module.items()):
        units_sorted = sorted(
            units, key=lambda u: (KIND_ORDER.get(u["kind"], 9), u["line_start"])
        )
